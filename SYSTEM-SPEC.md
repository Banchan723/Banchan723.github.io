...
> **핵심 철학: 설명하지 말고 기록한다.** 이 블로그의 글은 일반 튜토리얼이 아니라 **디버깅 로그**다.
> 비유보다 코드, 감상보다 출력, 일반론보다 막힌 지점을 남긴다.
> 초보 + AI가 쓰면 '그럴듯한 일반론'이 나와 신뢰가 안 간다. 운영자가 실제로 막힌 흔적·직접 친 코드·그 결과가 글의 원료여야 한다.

### (1) 제목 규칙

- **제목 = 개념명 그 자체.** 예: "C++ 포인터", "C++ 클래스", "Unreal 액터". 한글 가능, 항상 따옴표.
- **감성 비유 제목 금지** ("주소를 들고 다닌다는 것" 같은). **"완벽 정리"·"쉽게 이해하기"** 같은 교과서·과장 제목도 금지.
- **개념 1개 = 살아있는 학습 문서 1개.** 같은 개념(`canonical_topic`+`context` 동일)을 또 공부하면 새 글이 아니라 **기존 글에 누적**(5절의 "기존에 추가" 규칙과 동일). 새 개념이면 새 글.
- 이건 4절·5절의 중복 판정(`canonical_topic`+`context`) 시스템과 정확히 맞물린다. 제목이 개념명이므로 같은 개념은 자연히 같은 문서로 모인다.

### (2) 본문 골격 (디버깅 로그)

- **맨 위**: 학습노트 면책 한 줄 + (선택) 지금 내가 이해한 한 줄 정의.
- **학습 블록** (최초 1개, 추가 학습마다 같은 블록 append):
  - `## 내가 막혔던 점` — 실제 질문·착각. **필수**(AI가 못 지어냄. 비우지 말 것).
  - `## 확인한 코드` — 직접 친 최소 코드. 실행 코드 없는 개념글 금지.
  - `## 결과 / 동작` — 출력·에러·이상했던 현상. 환경 의존값(주소 등)은 지어내지 말고 **관계 중심**으로(예: `&x`와 `p`는 같고 `&p`는 다르다).
  - `## 그래서 이렇게 이해했다` — 정정된 이해 + 다음에 안 헷갈릴 내 규칙.
- **맨 끝**: `## 아직 모르는 것` — 다음에 더 팔 거리. 약점이 아니라 신뢰 장치다.
- **추가 학습 시**: `## 추가 학습 (YYYY-MM-DD)` 아래 같은 블록을 반복. 5회 넘으면 시리즈 분리(5절 규칙 유지).

### (3) 정확성 방어 (콘텐츠 차원)

- **단정문 줄이기.** "포인터는 ~이다"(일반론) 대신 "이번에 확인한 건 ~까지"(범위 한정)로 쓴다.
- **모르는 것은 숨기지 말고** `## 아직 모르는 것`에 적는다.
- **노션에 없는 지식을 AI가 임의로 추가하지 않는다.** 범위를 좁게 잡는다.
